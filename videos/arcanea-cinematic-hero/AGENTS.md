...
**Grok Build**: This dir has `.grok/hooks` (excellence json + sh) and `GROK.md`. Hooks auto-activate on `grok` launch (trust with /hooks-trust). They prevent the classic missed-instruction symptom: duplicated `tl.to` / captions.forEach blocks on the same targets. Lint is the hard requirement; validate timeouts are env-specific (headless WebGL).
