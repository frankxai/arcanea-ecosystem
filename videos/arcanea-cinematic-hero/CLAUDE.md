...
**Grok users**: Local `GROK.md` + `.grok/hooks` (session-start-excellence + pretooluse-excellence) are present. The hooks enforce rules-first + excellence (repo-mastery, no duplicate tweens, lint discipline). Read GROK.md on start. Validate may timeout in some headless envs (WebGL + CDNs + audio); lint is the non-negotiable gate for code correctness.
