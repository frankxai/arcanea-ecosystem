...
**Promotion checkpoint (2026-06-16):** The Living World engine is being promoted into `arcanea-ai-app` on gate branch `codex/world-engine-promotion`, path-scoped from `agent/claude/world-integration` instead of blind-merged. Approved app slice: `packages/world-sdk`, `surfaces/world-page`, `/api/worlds/genesis`, `/worlds/create`, and the fixture-backed `/worlds/the-drowned-archive` fallback. Keep branch hygiene strict: default branches stay clean, dirty local docs are preserved, Supabase remains a derived index/cache, and unrelated app/book/CI changes from the old integration base are not part of this promotion.

**Verification checkpoint:** Targeted gates now pass on Node 20 + `pnpm@8.15.0`: `@arcanea/world-sdk` is 21/21, the world-page renderer emits a 16 KB Drowned Archive page, `@arcanea/web` typecheck/typegen is clean, and webpack dev smoke returns 200 for `/worlds`, `/worlds/create`, and `/worlds/the-drowned-archive` with local dummy Supabase env. Full production build still requires real Supabase env and has a Turbopack/pnpm resolver debt outside the promotion slice.
